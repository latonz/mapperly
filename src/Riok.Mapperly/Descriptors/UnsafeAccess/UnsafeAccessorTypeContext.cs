...
public class UnsafeAccessorTypeContext(UniqueNameBuilder nameBuilder, INamedTypeSymbol type, SymbolAccessor symbolAccessor)
{
    ...
    internal MemberDeclarationSyntax BuildSyntax(SourceEmitterContext ctx, CancellationToken cancellationToken)
    {
        ...
        {
            // TODO
            var unboundType = type.ConstructedFrom;
            var syntax = (ClassDeclarationSyntax)unboundType.DeclaringSyntaxReferences[0].GetSyntax(cancellationToken);
            clazz = clazz.WithTypeParameterList(syntax.TypeParameterList).WithConstraintClauses(syntax.ConstraintClauses);
        }
