...
When source types come from a nullable-oblivious context (`#nullable disable` or `<Nullable>disable</Nullable>`),
Mapperly generates projection expressions without additional null guards for those oblivious members.
This helps ORMs such as EF Core translate projections without introducing unnecessary null-check branches.
